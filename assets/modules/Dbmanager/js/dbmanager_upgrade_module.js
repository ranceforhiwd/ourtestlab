 ...
 $(document).ready(function(){
    ...
    function compare_database(db){
        ...
        $.ajax({
            ...
            complete: function(){                
                $("div#dbCheck").append(resultsTable);                
                $("ul#add").append(a);
                $("ul#remove").append(r);
    ...
    function get_audit(){
        ...
        $.ajax({
                ...
                success: function(resp){                    
                    ...
                    $("div.update_status ul.update_status li div#row2.row").hide();
                    $("div.update_status ul.update_status li div#row3.row").html('<table id="changelog" class="dataTable no-footer" />');
                    $("table#changelog").append('<tr><th>General Updates</th><th>Primary Keys</th><th>Auto Increment Updates</th></tr>');
                    $("table#changelog").append('<tr class="updates"/>');
                    $("tr.updates").html('<td valign="top"><ul class="gen"></ul></td><td valign="top"><ul class="pri"></ul></td><td valign="top"><ul class="auto"></ul></td>');                     
                   
                    for(var gen in audit_log[0]){
                        $("tr.updates td ul.gen").append('<li style="color:#888888;list-style:none">'+audit_log[0][gen]+'</li>');
                    }
                    
                    for(var pri in audit_log[1]){
                        $("tr.updates td ul.pri").append('<li style="color:#888888;list-style:none">'+audit_log[1][pri]+'</li>');
                    }
                    
                    for(var auto in audit_log[2]){
                        $("tr.updates td ul.auto").append('<li style="color:#888888;list-style:none">'+audit_log[2][auto]+'</li>');
                    }                  
                },
                complete: function(){
                    clear_session_data();
                    clear_msg();
                    $("ul.update_status").append('<li style="color:green;list-style:none">Clean up process</li><br>');                     
                }
        });
    }
    ...
    function clear_session_data(){        
        $.ajax({
                ...
                success: function(resp){
                    console.log(resp);
                    $("ul.update_status").append("<li style='color:green;list-style:none'><button class='btn btn-sm btn-blue' id='finish' data-version="+resp+">Finish Upgrade</button></li>");                    
                }
        });
    }
    ...
    function run_upgrade(){
        $("div#dbmanager div.msg").text('Retrieving saved database structures...');
        $("div#controls").hide();       
        $("div#tableDetails").hide();
        $("div#keyDetails").hide();
        $("div#viewdetails").hide();
        $("a#updatedb").hide();
        $("a#finishdbupdate").hide();
